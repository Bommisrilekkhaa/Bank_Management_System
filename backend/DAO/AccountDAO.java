...
public class AccountDAO {
	...
	public int totalAccounts(Connection conn, HashMap<String, Integer> pathMap, String searchParam)
			throws SQLException {
		UserDAO userDAO = new UserDAO();
	...
	public boolean updateAccount(Connection conn, Account account) throws SQLException {
		...
		if (account.getAccStatus() == Status.INACTIVE.getValue()) {
			if (!updateBalance(conn, TransactionType.DEBIT.getValue(), BigDecimal.valueOf(1000.0),
					account.getAccNo())) {
				return false;
			}
		}
	...
	public boolean updateBalance(Connection conn, int type, BigDecimal amount, int acc_no) throws SQLException {
		...
		pathMap.put(Resources.ACCOUNTS.toString().toLowerCase(), acc_no);
		ResultSet rs = null;
		try {

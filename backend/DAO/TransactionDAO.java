...
public class TransactionDAO {
	...
	public boolean insertTransaction(Connection conn, Transaction transaction) throws SQLException {
		QueryUtil query = QueryUtil.create()
				.insert("transaction")
				.columns("transaction_datetime", "transaction_type", "transaction_status", "transaction_amount",
						"acc_number")
	...
	public String changeName(String key) {
		...
		} else if (key.equals(Resources.TRANSACTIONS.toString().toLowerCase())) {
	...
	public int totalTransactions(Connection conn, HashMap<String, Integer> pathMap, String searchParam)
			throws SQLException {
	...
	public ResultSet lastTransaction(Connection conn, HashMap<String, Integer> pathMap) throws SQLException {
		...
		conditions.put("acc_number", new Object[] { "=", pathMap.get(Resources.ACCOUNTS.toString().toLowerCase()) });

...
public class UserDAO {
				...
				if (rs.getInt("user_status") == (Integer) Status.ACTIVE.getValue()) {
					if (user.getUser_role() == (Integer) UserRole.ADMIN.getValue()) {
						...
						try {
							...
							rsBranch = branchDao.selectBranchByManager(conn, user.getUser_id());
							if (rsBranch.next()) {
	...
	public boolean registerUser(Connection conn, User user) throws SQLException {
		QueryUtil query = QueryUtil.create()
				.insert("users")
				.columns("full_name", "date_of_birth", "user_phonenumber", "user_address", "user_role", "username",
	...
	public ResultSet selectAllUsers(Connection conn, HashMap<String, Integer> pathMap) throws SQLException {
		Map<String, Object[]> conditions = new HashMap<>();

		if (pathMap.containsKey("users")) {
			conditions.put("user_id", new Object[] { "=", pathMap.get("users") });
		}
